...
function bookingCard(b){
  ...
  article.innerHTML=`<div class="booking-card-top"><div class="booking-service-identity"><i>⌂</i><div><span>${safe(b.bookingCode||'HomeServe booking')}</span><h3>${service}</h3></div></div><b class="booking-status-pill ${statusClass(status)}">${status}</b></div><div class="booking-information-grid"><div><small>Date</small><strong>${dateText(b.bookingDate)}</strong></div><div><small>Time slot</small><strong>${safe(b.timeSlot||'Not specified')}</strong></div><div><small>Professional</small><strong>${provider}</strong></div><div><small>Total</small><strong>${money(b.pricing?.total)}</strong></div></div><div class="booking-address"><small>Service address</small><p>${safe(b.address||'Not specified')}</p></div><div class="booking-progress-line"><span class="active"></span><span class="${['Accepted','On the Way','Completed'].includes(status)?'active':''}"></span><span class="${['On the Way','Completed'].includes(status)?'active':''}"></span><span class="${status==='Completed'?'active':''}"></span></div><div class="booking-progress-labels"><span>Created</span><span>Accepted</span><span>On the way</span><span>Completed</span></div><div class="booking-card-actions"><a href="contact.html" class="btn btn-secondary">Get support</a>${b.provider?`<a href="chat.html?booking=${encodeURIComponent(b._id||b.id)}" class="btn btn-secondary">Chat with worker</a>`:''}${canTrack?`<button class="btn btn-primary track-worker-btn" data-id="${b._id||b.id}">Track worker</button>`:''}${status==='Completed'?`<button class="btn btn-secondary invoice-booking-btn" data-id="${b._id||b.id}">Invoice</button><button class="btn btn-secondary review-booking-btn" data-id="${b._id||b.id}">${myReviews.has(String(b._id||b.id))?'Edit review':'Rate service'}</button>`:''}${status==='Completed' && b.payment?.status!=='Paid'?`<button class="btn btn-primary pay-booking-btn" data-id="${b._id||b.id}">Pay now</button>`:''}${canCancel?`<button class="cancel-booking-btn" data-id="${b._id||b.id}">Cancel booking</button>`:''}</div>`;
  ...
  article.querySelector('.invoice-booking-btn')?.addEventListener('click',()=>window.location.href=`invoice.html?id=${encodeURIComponent(b._id||b.id)}`);
  return article;
}
...
function openPayment(b){ paymentBooking=b; payModal.hidden=false; document.getElementById('customerPaymentTitle').textContent=b.service?.name||'Service payment'; const amount=paymentSettings.qrAmountMode==='fixed'?Number(paymentSettings.fixedQrAmount||1):Number(b.pricing?.total||b.service?.price||0); document.getElementById('customerPaymentAmount').textContent=`Amount: ₹${amount.toLocaleString('en-IN')}`; const method=document.getElementById('customerPaymentMethod'); method.value=b.payment?.method||'UPI QR'; updatePaymentFields(); const qr=document.getElementById('customerQrCode'); qr.innerHTML=''; if(paymentSettings.enabled&&paymentSettings.customQrImage){const img=document.createElement('img');img.src=paymentSettings.customQrImage;img.alt='Payment QR';img.style.maxWidth='180px';qr.appendChild(img);}else if(paymentSettings.enabled&&paymentSettings.upiId&&window.QRCode){new QRCode(qr,{text:`upi://pay?pa=${encodeURIComponent(paymentSettings.upiId)}&pn=${encodeURIComponent(paymentSettings.payeeName||'HomeServe')}&am=${amount.toFixed(2)}&cu=INR&tn=${encodeURIComponent((paymentSettings.paymentNote||'Home service payment')+' '+(b.bookingCode||''))}`,width:180,height:180});}}
