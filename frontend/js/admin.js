...
function renderRecent(items) {
  $("recentBookingsBody").innerHTML = items.length ? items.map(b => `<tr><td><strong>${shortId(b._id)}</strong></td><td>${escapeHtml(b.customer?.name || "Unknown")}</td><td>${escapeHtml(b.service?.name || "Removed service")}</td><td>${formatDate(b.bookingDate)}</td><td>${money(b.service?.price)}</td><td>${statusBadge(b.status)}</td></tr>`).join("") : `<tr><td colspan="6" class="empty-cell">No bookings yet.</td></tr>`;
}
...
function renderBookings() {
  ...
  $("bookingsBody").innerHTML = rows.length ? rows.map(b => `<tr>
    <td><strong>${shortId(b._id)}</strong><small>${money(b.service?.price)}</small></td>
...
function renderAdminQrPreview() {
  ...
  const uri = `upi://pay?pa=${encodeURIComponent(upi)}&pn=${encodeURIComponent($("paymentPayeeName").value.trim() || "HomeServe")}&am=1.00&cu=INR&tn=${encodeURIComponent($("paymentNote").value.trim() || "Home service payment")}`;
...
["paymentQrEnabled","allowProviderSelfAccept","paymentUpiId","paymentPayeeName","paymentNote"].forEach(id => $(id).addEventListener("input", renderAdminQrPreview));
...
async function bootstrapAdmin() {
  ...
  try {
    ...
    await loadUsers();
    await Promise.all([loadDashboard(), loadBookings(), loadPaymentSettings()]);
  } catch (error) {
