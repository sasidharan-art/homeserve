const REVIEW_API=`${window.location.origin}/api/reviews/provider-summary`;
...
let paymentSettings = { enabled:false, cashOnDeliveryEnabled:true, upiId:"", customQrImage:"", payeeName:"HomeServe", paymentNote:"Home service payment" };
...
function paymentButton(b) {
  if (b.status !== "Completed" || b.payment?.status === "Paid") return "";
  const hasUpi = paymentSettings.enabled && (paymentSettings.upiId || paymentSettings.customQrImage);
  const hasCash = paymentSettings.cashOnDeliveryEnabled !== false;
  if (!hasUpi && !hasCash) return "";
  return `<button type="button" data-payment-id="${escapeHtml(b._id)}">₹ Collect payment</button>`;
}

function paymentAmount(booking) {
  return paymentSettings.qrAmountMode === "fixed"
    ? Number(paymentSettings.fixedQrAmount || 1)
    : Number(booking.pricing?.total || booking.service?.price || 0);
}

function renderProviderPayment(method) {
  if (!activePaymentBooking) return;
  const isUpi = method === "UPI QR";
  $("chooseUpiPayment").classList.toggle("active", isUpi);
  $("chooseCashPayment").classList.toggle("active", !isUpi);
  $("providerQrWrap").hidden = !isUpi;
  $("providerCashWrap").hidden = isUpi;
  $("providerPaymentReferenceWrap").hidden = !isUpi;
  $("providerMarkPaid").disabled = false;
  $("providerPaymentStatus").textContent = activePaymentBooking.payment?.status || "Unpaid";

  if (!isUpi) return;
  const amount = paymentAmount(activePaymentBooking);
  $("providerQrCode").innerHTML = "";
  if (paymentSettings.customQrImage) {
    const img = document.createElement("img");
    img.src = paymentSettings.customQrImage;
    img.alt = "Payment QR";
    $("providerQrCode").appendChild(img);
  } else if (paymentSettings.upiId && window.QRCode) {
    const uri = `upi://pay?pa=${encodeURIComponent(paymentSettings.upiId)}&pn=${encodeURIComponent(paymentSettings.payeeName || "HomeServe")}&am=${amount.toFixed(2)}&cu=INR&tn=${encodeURIComponent(`${paymentSettings.paymentNote || "Home service payment"} ${activePaymentBooking.bookingCode || ""}`)}`;
  ...
  $("providerQrNote").textContent = paymentSettings.customQrImage ? "Admin-controlled payment QR · Ask the customer to scan and pay" : `UPI: ${paymentSettings.upiId} · Ask the customer to scan and pay`;
}

async function chooseProviderPaymentMethod(method) {
  if (!activePaymentBooking) return;
  const result = await request(`/bookings/${activePaymentBooking._id}/payment-method`, { method:"PATCH", body:JSON.stringify({ method }) });
  activePaymentBooking.payment = result.booking.payment;
  const local = allBookings.find(item => item._id === activePaymentBooking._id);
  if (local) local.payment = result.booking.payment;
  renderProviderPayment(method);
  toast(result.message);
}

function openProviderPayment(booking) {
  activePaymentBooking = booking;
  $("providerPaymentTitle").textContent = `${booking.service?.name || "Service"} payment`;
  $("providerPaymentReference").value = booking.payment?.transactionRef || "";
  $("providerPaymentStatus").textContent = booking.payment?.status || "Unpaid";
  $("chooseUpiPayment").hidden = !(paymentSettings.enabled && (paymentSettings.upiId || paymentSettings.customQrImage));
  $("chooseCashPayment").hidden = paymentSettings.cashOnDeliveryEnabled === false;
  const upiAvailable = paymentSettings.enabled && (paymentSettings.upiId || paymentSettings.customQrImage);
  const cashAvailable = paymentSettings.cashOnDeliveryEnabled !== false;
  const preferred = booking.payment?.method === "Cash on Delivery" && cashAvailable
    ? "Cash on Delivery"
    : upiAvailable ? "UPI QR" : "Cash on Delivery";
  renderProviderPayment(preferred);
...
function bookingCard(b) {
  ...
  return `<article class="provider-job-card" style="--status-color:${statusColor(b.status)}">
    ...
    <div class="job-secondary-actions">${locationButton(b)}${trackingButton(b)}${paymentButton(b)}<a class="provider-chat-link" href="chat.html?booking=${encodeURIComponent(b._id)}">Chat with customer</a></div>
...
$("bookingList").addEventListener("click", async event => {
  ...
  if (paymentBtn) {
    ...
    if (!booking) return toast("Booking not found.", true);
    openProviderPayment(booking);
    return;
  }
  ...
  try {
    ...
    if (button.dataset.nextStatus === "Completed") {
      const completedBooking = allBookings.find(item => item._id === button.dataset.bookingId) || result.booking;
      if (completedBooking) openProviderPayment(completedBooking);
    }
  } catch (err) {
...
$("chooseUpiPayment").addEventListener("click", async () => {
  try { await chooseProviderPaymentMethod("UPI QR"); } catch (err) { toast(err.message, true); }
});

$("chooseCashPayment").addEventListener("click", async () => {
  try { await chooseProviderPaymentMethod("Cash on Delivery"); } catch (err) { toast(err.message, true); }
});

$("providerMarkPaid").addEventListener("click", async () => {
  if (!activePaymentBooking) return;
  const button = $("providerMarkPaid");
  button.disabled = true;
  button.textContent = "Updating…";
  try {
    const result = await request(`/bookings/${activePaymentBooking._id}/mark-paid`, {
      method:"POST",
      body:JSON.stringify({ transactionRef:$("providerPaymentReference").value.trim() })
    });
    toast(result.message);
    ...
    button.textContent = "Mark unpaid as paid";
  }
});
...
async function loadProviderReviews(){
  const list=document.getElementById('providerReviewList');
  try{const r=await fetch(REVIEW_API,{headers:{Authorization:`Bearer ${token}`},cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.message||'Unable to load reviews');document.getElementById('providerAverageRating').textContent=Number(d.average||0).toFixed(1);document.getElementById('providerRatingBadge').textContent=d.count?`${d.average} ★`:'New';document.getElementById('providerReviewCount').textContent=d.count?`${d.count} verified review${d.count===1?'':'s'}`:'No reviews yet';list.innerHTML='';if(!d.recent?.length){list.innerHTML='<p class="empty-review">Complete jobs to begin receiving customer feedback.</p>';return;}d.recent.forEach(x=>{const item=document.createElement('article');item.className='provider-review-item';item.innerHTML=`<div><strong>${'★'.repeat(x.rating)}${'☆'.repeat(5-x.rating)}</strong><span>${x.service?.name||'Home service'}</span></div><p>${x.comment||'Customer left a rating without a written comment.'}</p><small>${x.customer?.name||'Customer'} · ${new Date(x.createdAt).toLocaleDateString('en-IN')}</small>`;list.appendChild(item);});}catch(e){if(list)list.innerHTML=`<p class="empty-review">${e.message}</p>`;}
}
loadProviderReviews();
if(window.socket) window.socket.on('review-updated',loadProviderReviews);
