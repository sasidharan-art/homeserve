...
function paymentButton(b) {
  if (!paymentSettings.enabled || !paymentSettings.upiId || b.status !== "Completed") return "";
  return `<button type="button" data-payment-id="${escapeHtml(b._id)}">▦ Payment QR</button>${paymentSettings.cashOnDeliveryEnabled && b.payment?.method === "Cash on Delivery" && b.payment?.status !== "Paid" ? `<button type="button" data-cash-id="${escapeHtml(b._id)}">₹ Cash collected</button>` : ""}`;
}
...
$("bookingList").addEventListener("click", async event => {
  ...
  const cashBtn = event.target.closest("[data-cash-id]");
  if (cashBtn) {
    try { await request(`/bookings/${cashBtn.dataset.cashId}/cash-collected`, { method:"POST" }); toast("Cash payment recorded."); await loadDashboard(); }
  ...
  if (paymentBtn) {
    ...
    const amount = paymentSettings.qrAmountMode === "fixed" ? Number(paymentSettings.fixedQrAmount || 1) : Number(booking.pricing?.total || booking.service?.price || 0);
